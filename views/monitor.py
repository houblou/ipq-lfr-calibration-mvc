...
class _DetailChart(tk.Frame):
    def __init__(self, parent, **kw):
        ...
        super().__init__(parent, highlightbackground=_C["border"], highlightthickness=1, **kw)
        self._titre = tk.Label(self, text="No run selected", font=_FB, bg=_C["card"], fg=_C["primary"], anchor="w")
        ...
        if _MPL:
            ...
            self._cv_tk = tk.Canvas(self, bg=_C["plot_bg"], highlightthickness=1, highlightbackground=_C["border"])
    ...
    def _dessiner_mpl(self, data: _SerieData) -> None:
        ...
        if valides:
            ...
            if res is not None:
                m, sigma = res
                ax.axhline(m, color=_C["violet"], linestyle="--", linewidth=1.2, label=f"M = {m:.5f}", zorder=2)
                if sigma > 0:
                    ax.axhspan(m - sigma, m + sigma, color=_C["violet"], alpha=0.10)
            # Dernier point mis en évidence
            ax.scatter([xs[-1]], [ys[-1]], s=70, facecolor=data.couleur, edgecolor="white", linewidths=1.5, zorder=4)
            ax.legend(fontsize=8, facecolor=_C["card"], labelcolor=_C["second"], edgecolor=_C["border"])
    ...
    def _dessiner_tk(self) -> None:
        ...
        valides = [(i + 1, v) for i, v in enumerate(data.vals) if v is not None]
        if not valides:
            c.create_text(W // 2, H // 2, text="No data", fill=_C["muted"], font=_F)
            return
        ...
        def px(i):
            return x0 + (i - 1) / max(self._nb_points - 1, 1) * (x1 - x0)

        def py(v):
            return y1 - (v - lo) / dy * (y1 - y0)

        for frac in range(5):
            yy = y1 - frac / 4 * (y1 - y0)
            c.create_line(x0, yy, x1, yy, fill=_C["grid"])
            c.create_text(x0 - 6, yy, anchor="e", fill=_C["muted"], font=_FS, text=f"{lo + frac / 4 * dy:.5f}")
        res = stats.moyenne_sigma(ys_v)
        if res is not None:
            m, sigma = res
            if sigma > 0:
                c.create_rectangle(x0, py(m + sigma), x1, py(m - sigma), fill="#efe7fb", outline="")
        ...
        c.create_text((x0 + x1) / 2, H - 6, anchor="s", fill=_C["second"], font=_FS, text="Sample index N[i]")


# readout horizon
class _ReadoutStrip(tk.Frame):
    CHAMPS = [("Active COM", "com"), ("Series", "serie"), ("Point i", "pt"), ("Temp.", "t"), ("Humidity", "hr"), ("Prov. M", "m"), ("Prov. V", "v")]
    ...
    def __init__(self, parent, **kw):
        ...
        for i, (titre, cle) in enumerate(self.CHAMPS):
            ...
            tk.Label(cell, text=titre.upper(), font=("Segoe UI", 8), bg=_C["card"], fg=_C["muted"]).pack(anchor="w", padx=12)
            var = tk.StringVar(value="—")
            self._vars[cle] = var
            tk.Label(cell, textvariable=var, font=_FM, bg=_C["card"], fg=_C["primary"]).pack(anchor="w", padx=12)
            if i < len(self.CHAMPS) - 1:
                tk.Frame(self, bg=_C["border"], width=1).grid(row=0, column=i, sticky="nse", pady=8)

    def update_stats(self, com: str, serie: str, i: int, t: float, hr: float, m_prov: Optional[float], v_prov: Optional[float]) -> None:
        self._vars["com"].set(label_multimetre(com) if com else "—")
        self._vars["serie"].set(serie)
        self._vars["pt"].set(f"{i} / {self._nb_points}")
        self._vars["t"].set(f"{t:g} °C")
        self._vars["hr"].set(f"{hr:g} %")
...
class _VueNuage(tk.Frame):
    def __init__(self, parent, **kw):
        super().__init__(parent, bg=_C["card"], highlightbackground=_C["border"], highlightthickness=1, **kw)
        ...
        if _MPL:
            ...
            tk.Label(self, text="matplotlib required for the cloud view.", bg=_C["card"], fg=_C["muted"], font=_F).pack(pady=30)
    ...
    def redessiner(self) -> None:
        ...
        if res is not None:
            m_g, sigma = res
            ax.axhline(m_g, color=_C["violet"], linestyle="--", linewidth=1, label=f"M = {m_g:.5f}")
            if sigma > 0:
                ax.axhspan(m_g - 2 * sigma, m_g + 2 * sigma, alpha=0.08, color=_C["violet"])
        # Auto-échelle Y : matplotlib ajuste aux données visibles (Δ min/max + marge).
        ax.set_xlim(0.5, self._nb_points + 0.5)
        ax.set_xlabel("Sample index N[i]", color=_C["second"], fontsize=9)
        ax.set_ylabel("Value", color=_C["second"], fontsize=9)
        if 0 < len(all_vals) and len(self._series) <= 12:
            ax.legend(fontsize=7, facecolor=_C["card"], labelcolor=_C["second"], edgecolor=_C["border"])
        self._cv.draw_idle()
...
class MonitorTab(tk.Frame):
    ...
    def _construire(self) -> None:
        ...
        liste = tk.Frame(body, bg=_C["card"], width=170, highlightbackground=_C["border"], highlightthickness=1)
        liste.pack(side="left", fill="y")
        liste.pack_propagate(False)
        tk.Label(liste, text="RUNS", font=_FB, bg=_C["card"], fg=_C["muted"], anchor="w").pack(fill="x", padx=12, pady=(10, 6))
        ...
        self._listbox = tk.Listbox(
            ...
            activestyle="none",
            selectbackground=_C["sel"],
            selectforeground=_C["violet"],
    ...
    def _build_header(self) -> None:
        hdr = tk.Frame(self, bg=_C["header"], highlightbackground=_C["border"], highlightthickness=1)
        hdr.pack(fill="x")

        tk.Label(hdr, text="Instrument monitor", font=_FB, bg=_C["header"], fg=_C["primary"]).pack(side="left", padx=(14, 10), pady=9)
        self._lbl_com = tk.Label(hdr, text=f"Measuring: {label_multimetre('com1')}", font=_FM, bg=_C["header"], fg=_C["second"])
    ...
    def _toggle_vue(self) -> None:
        if self._nuage.winfo_ismapped():
            ...
            self._detail.pack(fill="both", expand=True)
            self._btn_toggle.configure(text="☁  Cloud view", bg=_C["card"], fg=_C["second"], highlightthickness=1)
        else:
            self._detail.pack_forget()
            self._nuage.pack(fill="both", expand=True)
            self._btn_toggle.configure(text="▦  Grid view", bg=_C["violet"], fg="#ffffff", highlightthickness=0)
    ...
    def on_init_point(self, cible: str, i: int, val: Optional[float], t: float, hr: float) -> None:
        sid = f"init_{cible}"
        ...
        self._maj_ligne(sid)
        self._suivre(sid)
        self._readout.update_stats(cible, f"Init {label_multimetre(cible)}", i, t, hr, data.m_provisoire(), None)

    def on_init_complete(self, cible: str, m: Optional[float], v: Optional[float]) -> None:
        sid = f"init_{cible}"
    ...
    def on_serie_point(self, x: int, i: int, val: Optional[float], t: float, hr: float) -> None:
        ...
        self._readout.update_stats(self._com_actif.get(), serie_lbl, i, t, hr, data.m_provisoire(), None)
    ...
    def on_final_point(self, cible: str, i: int, val: Optional[float], t: float, hr: float) -> None:
        sid = f"final_{cible}"
        ...
        self._maj_ligne(sid)
        self._suivre(sid)
        self._readout.update_stats(cible, f"Final {label_multimetre(cible)}", i, t, hr, data.m_provisoire(), None)
